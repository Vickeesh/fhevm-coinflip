...
export default function CoinFlipDApp() {
    ...
    if (typeof window.ethereum !== 'undefined') {
      try {
        ...
        const contractInfoData: ContractInfo = {
          address: import.meta.env.VITE_CONTRACT_ADDRESS || '0x2B5C509634992c78Ea5103a3401d8F26d2486841',
        ...
        setConnectionStep('Initializing FHEVM v0.9...');
        console.log('Initializing FHEVM v0.9 instance...');
        
        const fhevmInst = await initializeFhevmInstance(
          Number(network.chainId),
          import.meta.env.VITE_RPC_URL || 'https://ethereum-sepolia.publicnode.com',
          import.meta.env.VITE_FHEVM_GATEWAY_URL || 'https://gateway.sepolia.zama.ai'
        );
        
        if (fhevmInst) {
          console.log('FHEVM v0.9 instance initialized successfully');
        } else {
          console.warn('FHEVM v0.9 initialization failed, some features may not work');
        }
  ...
  const flipCoin = async (): Promise<void> => {
    ...
    if (!fhevmInstance) {
      showError('contract', 'FHEVM not initialized. Please reconnect your wallet.', true);
      return;
    }
    ...
    try {
      ...
      console.log('FHEVM v0.9 - Creating game with encrypted randomness');
      console.log('Choice:', choice, '| Bet:', betAmount, 'ETH');
      
      // Step 1: Create game and generate encrypted random
      ...
      // Find the GameCreated event to get gameId
      ...
      // Step 2: Automatically decrypt and resolve
      await decryptAndResolveGame(gameId);
      ...
      if (err.code === 'ACTION_REJECTED') {
        showError('transaction', 'Transaction was rejected by user.', true);
      } else if (err.code === 'INSUFFICIENT_FUNDS') {
        showError('transaction', 'Insufficient funds for this bet.', false);
      } else {
        showError('transaction', `Failed to create game: ${err.message || 'Unknown error'}`, true);
      }
    }
  };

  const decryptAndResolveGame = async (gameId: number): Promise<void> => {
    if (!contract || !fhevmInstance || !contractInfo) {
      showError('contract', 'Contract or FHEVM not ready', false);
      return;
    }

    try {
      setIsDecrypting(true);
      console.log('\n=== FHEVM v0.9 Decryption Process ===' );
      console.log('Step 1: Fetching ciphertext handle...');
      
      // Get the ciphertext handle from contract
      const ciphertextHandle = await contract.getCiphertextHandle(gameId);
      console.log('Ciphertext handle:', ciphertextHandle.toString());
      
      console.log('Step 2: Performing off-chain decryption...');
      
      // Perform off-chain decryption
      const { cleartext, proof } = await publicDecrypt(
        fhevmInstance,
        contractInfo.address,
        BigInt(ciphertextHandle.toString())
      );
      
      console.log('Decrypted result:', cleartext, '(0=tails, 1=heads)');
      console.log('Proof:', proof);
      
      setIsDecrypting(false);
      setIsResolving(true);
      
      console.log('Step 3: Submitting decryption proof to contract...');
      
      // Resolve the game on-chain with the decrypted value and proof
      const resolveTx = await contract.resolveGame(gameId, cleartext, proof);
      ...
      const gameResolvedEvent = resolveReceipt.logs.find(
      ...
      if (gameResolvedEvent) {
        ...
        const outcome: 'heads' | 'tails' = cleartext === 1 ? 'heads' : 'tails';
        ...
        if (won) {
          showSuccess('win', `🎉 You won ${payoutEth.toFixed(4)} ETH! (FHEVM v0.9 Encrypted)`);
        }
      ...
      console.error('Error in decryption/resolution:', err);
      setIsDecrypting(false);
      setIsResolving(false);
      setPendingGameId(null);
      
      showError('transaction', `Decryption failed: ${err.message || 'Unknown error'}`, true);
    }
  };
      ...
      <div className="relative z-10 max-w-7xl mx-auto px-4 py-6">
            ...
            <div className="bg-slate-900 rounded-xl border border-slate-800 overflow-hidden">
              ...
              <div className="p-8">
                ...
                <div className="flex justify-center items-center h-64 mb-8 bg-gradient-to-br from-slate-800/30 to-slate-900/30 rounded-xl border border-slate-700/50 relative overflow-hidden">
                  ...
                  <div className={`relative transition-all duration-700 ${(isFlipping || isDecrypting || isResolving) ? 'animate-spin scale-110' : 'scale-100'}`}>
                    <div className="w-40 h-40 rounded-full bg-gradient-to-br from-slate-700 via-slate-600 to-slate-800 flex items-center justify-center shadow-2xl border-4 border-slate-500 relative">
                      ...
                      <span className="text-6xl font-bold text-slate-100 relative z-10 drop-shadow-lg">
                        {isFlipping ? '🎲' : isDecrypting ? '🔐' : isResolving ? '⚙️' : result ? (result.outcome === 'heads' ? 'H' : 'T') : 'Ξ'}
                      </span>
                    </div>
                    
                    {/* Glow effect when processing */}
                    {(isFlipping || isDecrypting || isResolving) && (
                ...
                <div className="space-y-6">
                  ...
                  <button
                    onClick={flipCoin}
                    disabled={isFlipping || isDecrypting || isResolving || !walletConnected}
        ...
        <footer className="mt-8 pt-6 border-t border-slate-800 text-center">
          <p className="text-sm text-slate-500">
            ...
            {contractInfo?.fhevmEnabled && (
              <span className="ml-2 text-purple-400">🔐 FHEVM v0.9 Encrypted</span>
            )}
            {!contractInfo?.fhevmEnabled && (
