...
/**
 * FHEVM v0.9 Test Suite
 * 
 * Note: These tests use mock decryption since true FHEVM decryption requires
 * the Zama KMS and relayer infrastructure. In production, the decryption
 * would happen off-chain via the relayer SDK.
 */

describe("FHECoinFlip - FHEVM v0.9", function () {
  let fheCoinFlip;
  ...
  /**
   * Mock decryption helper for testing
   * In production, this would be done off-chain via FHEVM relayer SDK
   */
  async function mockResolveGame(gameId, mockResult = 0) {
    // Mock ABI-encoded cleartext (simplified for testing)
    const mockAbiEncodedCleartext = ethers.AbiCoder.defaultAbiCoder().encode(["uint8"], [mockResult]);
    
    // Mock proof - in production this comes from Zama KMS
    const mockProof = "0x";
    
    // Call resolveGame with mocked decrypted result
    return await fheCoinFlip.resolveGame(gameId, mockResult, mockAbiEncodedCleartext, mockProof);
  }
  ...
  describe("Game Creation and Coin Flip (FHEVM v0.9 Two-Step)", function () {
    beforeEach(async function () {
    ...
    it("Should create a game without immediate resolution (FHEVM v0.9)", async function () {
      ...
      // Check GameReadyForDecryption event (new in v0.9)
      const readyEvent = receipt?.logs.find(
        (log) => log.fragment && log.fragment.name === "GameReadyForDecryption"
      );
      ...
      // Verify game is NOT resolved yet (key difference from v0.8)
      const game = await fheCoinFlip.getGame(0);
      expect(game.isResolved).to.be.false;
      expect(game.isDecryptable).to.be.true;
    });
    ...
    it("Should create a game with valid bet (Heads)", async function () {
      ...
      // Step 2: Resolve game (would be done by user in FHEVM v0.9)
      // Using mock result: 1 = heads (player chose heads, so wins)
      await mockResolveGame(0, 1);
      
      // Verify game is now resolved
      const game = await fheCoinFlip.getGame(0);
    ...
    it("Should create a game with valid bet (Tails)", async function () {
      ...
      // Resolve with tails (0 = tails)
      await mockResolveGame(0, 0);
      
      const game = await fheCoinFlip.getGame(0);
      expect(game.isResolved).to.be.true;
      expect(game.won).to.be.true; // Player chose tails, result is tails
    });

    /**
     * NOTE: Many tests below need updating for FHEVM v0.9's two-step architecture.
     * Tests that expect immediate resolution will fail because games now require:
     * 1. flipCoin() to create game
     * 2. resolveGame() to decrypt and settle
     * 
     * For full migration, update all tests to use mockResolveGame() helper.
     */

    it("Should create game and resolve with two-step process (FHEVM v0.9)", async function () {
      const betAmount = hre.ethers.parseEther("0.01");
      ...
      // Game should NOT be resolved yet
      const gameBeforeResolve = await fheCoinFlip.getGame(0);
      expect(gameBeforeResolve.isResolved).to.be.false;
      expect(gameBeforeResolve.isDecryptable).to.be.true;
      
      // Step 2: Resolve game
      await mockResolveGame(0, 1); // Simulate decryption returning 1 (heads)

      // Check for GameResolved event after resolution
      const game = await fheCoinFlip.getGame(0);
      expect(game.isResolved).to.be.true;
    ...
    it("Should store game data correctly", async function () {
      ...
      // Resolve the game
      await mockResolveGame(0, 0); // Tails = 0
  ...
  describe("FHEVM Encrypted Randomness", function () {
    it("Should generate different results across multiple games", async function () {
      ...
      for (let i = 0; i < 10; i++) {
        ...
        // Resolve each game with mock random result (simulate FHEVM randomness)
        const mockResult = i % 2; // Alternate between 0 and 1 to simulate randomness
        await mockResolveGame(i, mockResult);
